...
The main model treats $\rho$ as exogenous. This section asks: why do agents converge on a common model, and can they stop? The coordination failure in AI adoption resembles the technology adoption externalities studied by \citet{katz1985} and \citet{farrell1985}, with the distinction that the externality here operates through signal correlation rather than network compatibility. The excess inertia result of \citet{farrell1985}, where an installed base prevents efficient switching, maps to the equilibrium lock-in at $\rho^{\text{NE}} > \rho^*$ in Proposition~\ref{prop:endogenous-rho}. \citet{gans2023} shows AI adoption in product markets generates non-standard externalities; the financial-market analog here is the systemic fragility externality. We endogenise $\rho$ by modelling fund managers' AI adoption decisions as a strategic game. Each manager has a private incentive to adopt the dominant AI model. The aggregate effect of adoption raises effective correlation above the socially optimal level, activating the fragility channels of Propositions~\ref{prop:uniqueness-boundary}--\ref{prop:bifurcation}. The answer is a prisoner's dilemma.
...
\end{equation}
increasing in the deviation $(1-\rho_i)$ from the AI signal and in $\bar{\rho}$ (when most managers use the AI model, the benchmark is the AI signal).\footnote{The multiplicative form of the tracking error cost ensures strategic complementarity. The qualitative result $\rho^{\text{NE}} > \rho^{\text{SO}}$ holds for any specification generating strategic complementarity in adoption, including additive costs with sufficient cross-partial in $(\rho_i, \bar{\rho})$.} Third, a systemic risk externality from the amplification loop:
...
Diversity requirements that bound aggregate signal correlation below $\rho^*$ are welfare-improving under the stated conditions. Alternative instruments include Pigouvian taxes on signal correlation (charging institutions proportional to their contribution to aggregate $\bar{\rho}$), subsidies to private fundamental research (reducing $c_P$ to restore the competitive viability of uncorrelated signals), and position limits calibrated to $N_{\text{eff}}$ (restricting correlated inventory exposure). Each instrument faces implementation challenges: direct enforcement of diversity requirements requires observing each manager's AI model weight $\rho_i$ through portfolio disclosure or model audit; Pigouvian taxes require measuring pairwise signal correlation in real time; research subsidies require identifying genuinely private research. The optimal regulatory instrument depends on observability and enforcement costs not modelled here.
