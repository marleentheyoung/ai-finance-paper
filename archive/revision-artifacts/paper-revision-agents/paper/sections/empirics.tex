...
\input{tables/rho_proxy_descriptive}
...
The model generates three specific predictions that can guide future empirical work.

\emph{Prediction 1 (Non-monotone crisis probability).} As AI model overlap increases, measured by portfolio return correlation or forecast revision correlation across institutions, crisis probability should first decrease then increase. The non-monotonicity of Proposition~\ref{prop:crisis-nonmonotone} implies a hump-shaped relationship. This is testable via a panel regression of crisis indicators (bank CDS spreads, systemic risk measures such as CoVaR or SRISK) on a quadratic function of the $\rho$ proxy, using CRSP and 13F data. The distinguishing prediction relative to \citet{danielsson2025}, whose $\theta^*(\mu)$ is monotonically increasing, is the initial decline in crisis risk at low $\rho$.

\emph{Prediction 2 (FPE-RPE divergence).} AI adoption should improve forecast accuracy (FPE) while reducing the information content of prices for real decisions (RPE). This is testable by comparing analyst forecast accuracy from I/B/E/S with real investment sensitivity to prices, following \citet{bond2012}. A declining ratio of investment-price sensitivity to forecast accuracy would signal increasing redundancy in price signals. Cross-sectional variation in AI adoption intensity across firms provides the identifying variation.

\emph{Prediction 3 ($N_{\text{eff}}$ collapse).} Markets with higher AI penetration among market makers should exhibit higher bid-ask spread comovement, reflecting the reduction in effective independent liquidity providers. This is testable using TAQ data on bid-ask spreads and cross-sectional variation in algorithmic market-making share. The model predicts that the pairwise correlation of spread changes across market makers should be increasing in the degree of AI model overlap, with the relationship being convex (Proposition~\ref{prop:neff}).
